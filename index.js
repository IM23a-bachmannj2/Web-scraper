...
export class BrowserFetchError extends Error {
    constructor() {
        super("Die Webseite konnte nicht direkt im Browser geladen werden. Auf GitHub Pages scheitern viele Seiten ohne CORS-Freigabe.");
    }
}
export async function analyzeWebsite(url, fetchImplementation = fetch) {
...
async function analyzePage(url, fetchImplementation) {
...
async function fetchHtml(url, fetchImplementation) {
    let response;
    try {
...
async function analyzeLinkedPages(links, baseUrl, fetchImplementation) {
...
function extractMetaDescription(parsedDocument) {
...
function extractHtmlLanguage(parsedDocument) {
    return toNullableText(parsedDocument.documentElement.getAttribute("lang"));
}
function extractTopHeadings(parsedDocument) {
...
function extractTextSample(parsedDocument, maxLength) {
...
function extractLinksFromDocument(parsedDocument, baseUrl) {
    const links = [];
    const seen = new Set();
...
export function splitLinksByOrigin(links, baseUrl) {
...
function getAppElements(root) {
    ...
    return {
        ...
        error,
        results,
        submitButton: submitButton instanceof HTMLButtonElement || submitButton instanceof HTMLInputElement ? submitButton : null,
    };
}
...
function renderRecursivePage(linkedPage) {
    ...
    const headingList = linkedPage.topHeadings.length > 0
...
function summarizeLinkedPages(linkedPages) {
    return linkedPages.reduce((summary, linkedPage) => {
        const pageUrl = linkedPage.finalUrl ?? linkedPage.url;
        const { internalLinks, externalLinks } = splitLinksByOrigin(linkedPage.links, pageUrl);
        return {
